...
public class SwerveModule {
 ...
 private static final double MAX_VELOCITY_METERS_PER_SEC = 4.46 * 0.9;
    static {
        ...
        DRIVING_CONFIG.encoder.positionConversionFactor(DRIVE_MOTOR_REDUCTION*DRIVE_WHEEL_CIRCUMFERANCE_METERS).velocityConversionFactor(DRIVE_MOTOR_REDUCTION*DRIVE_WHEEL_CIRCUMFERANCE_METERS/60);
        DRIVING_CONFIG.closedLoop.pid(0.04, DRIVE_MOTOR_REDUCTION, DRIVE_MOTOR_PINION_TEETH).feedForward.kV(MAX_VELOCITY_METERS_PER_SEC/12);
    ...
    public SwerveModuleState getState () {
        return new SwerveModuleState(driving.getEncoder().getVelocity(), new Rotation2d(steering.getAbsoluteEncoder().getPosition() - offset));
        
    }
    public SwerveModulePosition getPosition () {
        return new SwerveModulePosition(driving.getEncoder().getPosition(), new Rotation2d(steering.getAbsoluteEncoder().getPosition() - offset));
    } 
    public void SetDesiredState (SwerveModuleState desiSwerveModuleState) {
        SwerveModuleState rotated = new SwerveModuleState(desiSwerveModuleState.speedMetersPerSecond, desiSwerveModuleState.angle.plus(new Rotation2d(offset)));
